...
/*WARNING!!!
The original author is using a pointer variable to store the value of 
an integer in this program and while it does work (a pointer is just an integer),
it will blow up if you ever try to dereference it. This is because the value
being stored in the pointer isn't a valid address, it's the value of an
index variable. The compiler gives the appropriate warnings. 
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
	...
	*/
	switch (token) {
		...
		if ((np = search_symtab(word_string, symtab_root))
		    ...
		    np->info = (char *) index++; /* BAD!! using a pointer variable to hold the value of an integer */
		}
		break;
...

{
    ...
    */
    ...
    index  = (short) np->info; /* BAD!! using a pointer variable to hold the value of an integer */
    fwrite(&index,  sizeof(short), 1, crunch_file);
